...
if __name__ == '__main__':
    ...
    parser.add_argument('--algo', type=str, choices={'ac', 'mfac', 'mfq', 'il', 'rnnIL', 'rnnMFQ', 'pomfqv1', 'pomfq'}, help='choose an algorithm from the preset', required=True)
    parser.add_argument('--oppo', type=str, choices={'ac', 'mfac', 'mfq', 'il', 'rnnIL', 'rnnMFQ', 'pomfqv1',  'pomfq'}, help='indicate the opponent model')
    ...
    parser.add_argument('--pomfqv1_position', type=int, default=2, help='set the position of pomfqv1 if no pomfqv1 - position is 2')
    ...
    pomfqv1_position = args.pomfqv1_position

    with open('storepoints.csv', 'w+') as myfile:
        myfile.write('{0},{1},{2}\n'.format("Game", "Reward 1", "Reward 2"))
    
    with open('numberofwins.csv', 'w+') as myfile:
        myfile.write('{0},{1},{2}\n'.format("Game", "nw1", "nw2"))
    
    tf_config = tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)
    tf_config.gpu_options.allow_growth = True

    main_model_dir = os.path.join(BASE_DIR, 'data/battle/models/{}-0'.format(args.algo))
    oppo_model_dir = os.path.join(BASE_DIR, 'data/battle/models/{}-1'.format(args.oppo))
    ...
    runner = tools.Runner(sess, env, handles, args.map_size, args.max_steps, models, battle, pomfq_position, pomfqv1_position, render_every=0)
    win_cnt = {'main': 0, 'opponent': 0}

    for k in range(0, args.n_round):
        total_rewards = runner.run(0.0, k, win_cnt=win_cnt)
        with open('storepoints_battlepomfqvspomfqv1.csv', 'a') as myfile:
            myfile.write('{0},{1},{2}\n'.format(k, total_rewards[0], total_rewards[1]))
    
        with open('numberofwins_battlepomfqvspomfqv1.csv', 'a') as myfile:
