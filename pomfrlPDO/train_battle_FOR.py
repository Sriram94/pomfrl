...
from examples.battle_model.senario_battlev1 import play2
from examples.battle_model.senario_battlev1 import play
...
if __name__ == '__main__':
    ...
    parser.add_argument('--algo', type=str, choices={'ac', 'mfac', 'mfq', 'il', 'pomfq', 'pomfqv1'}, help='choose an algorithm from the preset', required=True)
    ...
    log_dir = os.path.join(BASE_DIR,'data/battle/tmp'.format(args.algo))
    model_dir = os.path.join(BASE_DIR, 'data/battle/models/{}'.format(args.algo))

    with open('pomfqv1_multibattle.csv', 'w+') as myfile:
        myfile.write('{0},{1}\n'.format("Episode", "Reward"))
    
    if args.algo in ['mfq', 'mfac', 'pomfqv1', 'pomfq']:
        use_mf = True
    else:
        use_mf = False
    ...
    for k in range(start_from, start_from + args.n_round):
        ...
        with open('pomfqv1_multibattle.csv', 'a') as myfile:
