...
class EpisodesBufferEntry:
    ...
    def append(self, feature, feature2, action, reward, alive, probs=None):
...
class EpisodesBuffer(Buffer):
    ...
    def push(self, **kwargs):
        ...
        for i in range(len(ids)):
            ...
            if self.use_mean:
                entry.append(feature[i], feature2[i], acts[i], rewards[i], alives[i], probs=probs[i])
            else:
                entry.append(feature[i], feature2[i], acts[i], rewards[i], alives[i])
...
class Runner(object):
    def __init__(self, sess, env, handles, map_size, max_steps, models,
                play_handle, pomfq_position = 2, pomfqv1_position = 2, pursuit=False, render_every=None, save_every=None, tau=None, log_name=None, log_dir=None, model_dir=None, train=False):
        """Initialize runner
        ...
        self.pomfqv1_position = pomfqv1_position
    ...
    def run(self, variant_eps, iteration, win_cnt=None):
        ...
        max_nums, nums, agent_r_records, total_rewards = self.play(env=self.env, n_round=iteration, map_size=self.map_size, max_steps=self.max_steps, handles=self.handles,
                    models=self.models, pomfq_position = self.pomfq_position, pomfqv1_position = self.pomfqv1_position, print_every=50, eps=variant_eps, render=(iteration + 1) % self.render_every if self.render_every > 0 else False, train=self.train)
