...
class ValueNet:
    def __init__(self, sess, env, handle, name, update_every=5, use_mf=False, learning_rate=1e-4, tau=0.005, gamma=0.95, maxelements=20):
        ...
        with tf.variable_scope(name or "ValueNet"):
            ...
            if self.use_mf:
                ...
                self.act_prob_input1 = tf.placeholder(tf.float32, (None,) + self.lambda_space, name="Act-lambda-input")
    ...
    def _construct_net(self, active_func=None, reuse=False):
        ...
        if self.use_mf:
            ...
            prob_emb1 = tf.layers.dense(self.act_prob_input1, units=64, activation=active_func, name='Prob-Emb1')
            h_act_prob1 = tf.layers.dense(prob_emb1, units=32, activation=active_func, name="Dense-Act-Prob1")
            concat_layer = tf.concat([concat_layer, h_act_prob1], axis=1)
    ...
    def calc_target_q(self, **kwargs):
        ...
        kwargs: {'feature', 'feature2', 'feature3', 'prob0', 'prob1', 'dones', 'rewards'}
        """
        feed_dict = {
    ...
    def act(self, **kwargs):
        """Act
        kwargs: {'feature', 'feature2', 'feature3', 'prob0', 'prob1', 'eps'}
        """
        feed_dict = {
    ...
    def train(self, **kwargs):
        """Train the model
        kwargs: {'state': [feature, feature2, feature3], 'target_q', 'prob0', 'prob1', 'acts'}
        """
        feed_dict = {
            self.feat_input: kwargs['state'][0],
            self.new_input: kwargs['state'][1],
