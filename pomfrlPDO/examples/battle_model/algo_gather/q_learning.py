...
class POMFQ(base2.ValueNet):
    ...
    def train(self):
        ...
        for i in range(batch_name):
            feat, feat2, feat3, acts, act_prob0, act_prob1, feat_next, feat2_next, feat3_next, act_prob0_next, act_prob1_next, rewards, dones, masks = self.replay_buffer.sample()
            target_q = self.calc_target_q(feature=feat_next, feature2=feat2_next, feature3=feat3_next, rewards=rewards, dones=dones, prob0=act_prob0_next, prob1=act_prob1_next)
            loss, q = super().train(state=[feat, feat2, feat3], target_q=target_q, prob0=act_prob0, prob1 = act_prob1, acts=acts, masks=masks)
