...
from examples.battle_model.senario_pursuit import play
from examples.battle_model.senario_pursuit import play2
...
if __name__ == '__main__':
    ...
    parser.add_argument('--algo', type=str, choices={'ac', 'mfac', 'mfq', 'il','rnnIL', 'rnnMFQ', 'pomfq'}, help='choose an algorithm from the preset', required=True)
    parser.add_argument('--save_every', type=int, default=500, help='decide the self-play update interval')
    ...
    parser.add_argument('--map_size', type=int, default=40, help='set the size of map')  # then the amount of agents is 50
    ...
    log_dir = os.path.join(BASE_DIR,'data/pursuit/tmp'.format(args.algo))
    model_dir = os.path.join(BASE_DIR, 'data/pursuit/models/{}'.format(args.algo))

    with open('pursuit.csv', 'w+') as myfile:
        myfile.write('{0},{1},{2}\n'.format("Episode", "Reward", "Reward(prey)"))
    
    if args.algo in ['mfq', 'mfac', 'pomfq', 'rnnMFQ']:
        use_mf = True
    else:
        use_mf = False
    ...
    for k in range(start_from, start_from + args.n_round):
        ...
        with open('pursuit.csv', 'a') as myfile:
            myfile.write('{0},{1},{2}\n'.format(k, total_reward[0], total_reward[1]))
