...
class MFAC:
    def __init__(self, sess, name, handle, env, value_coef=0.1, ent_coef=0.08, gamma=0.95, batch_size=64, learning_rate=1e-4, maxelements = 20, food = 180):
        self.sess = sess
        self.env = env
        self.name = name
        ...
        self.gather_space = (gather_count,)
        self.value_coef = value_coef  # coefficient of value in the total loss
        self.ent_coef = ent_coef  # coefficient of entropy in the total loss

        # init training buffers
        self.feature_buf = np.empty((1,) + self.feature_space)
        self.feature2_buf = np.empty((1,) + self.new_space)
        self.feature3_buf = np.empty((1,) + self.gather_space)
        self.action_buf = np.empty(1, dtype=np.int32)
        self.reward_buf = np.empty(1, dtype=np.float32)
        ...
        with tf.variable_scope(name):
            self.name_scope = tf.get_variable_scope().name
            self._create_network(self.feature_space, self.new_space, self.gather_space)
    ...
    def act(self, **kwargs):
        action = self.sess.run(self.calc_action, {
            self.input_feature: kwargs['state'][0],
            self.input_feature2: kwargs['state'][1],
            self.input_feature3: kwargs['state'][2],
            self.input_act_prob: kwargs['prob']

        })
        return action.astype(np.int32).reshape((-1,))

    def _create_network(self, feature_space, new_space, gather_space):
        # input
        input_feature = tf.placeholder(tf.float32, (None,) + feature_space)
        input_feature2 = tf.placeholder(tf.float32, (None,) + new_space)
        input_feature3 = tf.placeholder(tf.float32, (None,) + gather_space)
        ...
        h_emb = tf.layers.dense(input_feature,  units=hidden_size[0], activation=tf.nn.relu)
        h_emb2 = tf.layers.dense(input_feature2,  units=hidden_size[0], activation=tf.nn.relu)
        ...
        self.input_feature3 = input_feature3
    ...
    def train(self):
        ...
        self.feature_buf.resize((n,) + self.feature_space)
        self.feature2_buf.resize((n,) + self.new_space)
        self.feature3_buf.resize((n,) + self.gather_space)
        self.action_buf.resize(n)
        self.reward_buf.resize(n)
        feature, feature2, feature3 = self.feature_buf, self.feature2_buf, self.feature3_buf
        action, reward = self.action_buf, self.reward_buf
        ...
        for k, episode in enumerate(batch_data):
            f, f2, f3, a, r, prob = episode.features, episode.features2, episode.features3, episode.actions, episode.rewards, episode.probs
            m = len(episode.rewards)
            ...
            keep = self.sess.run(self.value, feed_dict={
                self.input_feature: [f[-1]],
                self.input_feature2: [f2[-1]],
                self.input_feature3: [f3[-1]],
        ...
        _, pg_loss, vf_loss, ent_loss, state_value = self.sess.run(
            [self.train_op, self.pg_loss, self.vf_loss, self.reg_loss, self.value], feed_dict={
                self.input_feature: feature,
                self.input_feature2: feature2,
                self.input_feature3: feature3,
