...
class MFAC:
    def __init__(self, sess, name, handle, env, value_coef=0.1, ent_coef=0.08, gamma=0.95, batch_size=64, learning_rate=1e-4, maxelements = 20):
        self.sess = sess
        self.env = env
        self.name = name
        ...
        # init training buffers
        self.feature_buf = np.empty((1,) + self.feature_space)
        self.feature2_buf = np.empty((1,) + self.feature_space)
        self.action_buf = np.empty(1, dtype=np.int32)
        self.reward_buf = np.empty(1, dtype=np.float32)
        ...
        with tf.variable_scope(name):
            self.name_scope = tf.get_variable_scope().name
            self._create_network(self.feature_space, self.new_space)
    ...
    def _create_network(self, feature_space, new_space):
        # input
        input_feature = tf.placeholder(tf.float32, (None,) + feature_space)
    ...
    def train(self):
        ...
        self.action_buf.resize(n)
        self.reward_buf.resize(n)
        feature, feature2 = self.feature_buf, self.feature2_buf
        action, reward = self.action_buf, self.reward_buf
        ...
        for k, episode in enumerate(batch_data):
            f, f2, a, r, prob = episode.features, episode.features2, episode.actions, episode.rewards, episode.probs
            m = len(episode.rewards)
