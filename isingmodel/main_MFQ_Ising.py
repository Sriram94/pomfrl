...
parser.add_argument('-ts', '--time_steps', default=20000, type=int)
...
with open('isingmodel.csv', 'w+') as myfile:
    myfile.write('{0},{1}\n'.format("Episode", "mse"))

def boltzman_explore(Q, temper, state, agent_index):
  action_probs_numes = []
  denom = 0
  #print("The Q value is", Q[agent_index, state])
  ...
  #print("The action_probs are", action_probs)
...
for i_episode in range(n_episode):
  ...
  for t in range(max_steps):
    ...
    mse /= n_agents
    obs = obs_
    with open('isingmodel.csv', 'a') as myfile:
       myfile.write('{0},{1}\n'.format(t, mse))
